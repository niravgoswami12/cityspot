...
public final class LocationUtility {
	...
	public static String getDistanceString(double distance, boolean useMetricSystem) {
		...
		if (useMetricSystem) {
			if (distance < 1000.0) {
				result = String.format(Locale.US, "%d " + CitySpotApplication.getContext().getString(R.string.unit_meter), (int) distance);
			} else if (distance < 5000.0) {
				result = String.format(Locale.US, "%.1f " + CitySpotApplication.getContext().getString(R.string.unit_kilometer), distance / 1000);
			} else {
			...
			if (distanceMiles < 0.1) {
				...
				result = String.format(Locale.US, "%.1f " + CitySpotApplication.getContext().getString(R.string.unit_mile), distanceMiles);
			} else {
				result = String.format(Locale.US, "%d " + CitySpotApplication.getContext().getString(R.string.unit_mile), (int) distanceMiles);
			}
		}
